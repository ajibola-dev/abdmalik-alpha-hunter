...
def extract_project_names(tweet_text: str,
    ...
    if candidates:
        ...
        for c in candidates:
            ...
            cl = c.lower()
            # Quality checks on list items:
            # - min 2 chars, max 30 chars
            # - not a noise word
            # - not a sentence fragment (contains a space but looks like phrase)
            # - not 'isthis', 'working?' style test strings
            if not c or len(c) < 2 or len(c) > 35:
                continue
            if cl in _NOISE_WORDS:
                continue
            # Reject if it contains question marks or looks like a sentence
            if '?' in c or cl.startswith('is') and len(c) > 8:
                continue
            # Reject multi-word items that look like sentences not project names
            words = c.split()
            if (len(words) > 3 and
                    not any(w[0].isupper() for w in words if len(w) > 2)):
                continue
            if cl not in seen:
                seen.add(cl)
                cleaned.append(c)
